...
import { googleCalendarOAuthConfigured, googleCalendarRedirectUri } from '@/lib/google/google-calendar-config';
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    debug: {
      clientIdPresent: Boolean(process.env.GOOGLE_CALENDAR_CLIENT_ID?.trim()),
      redirectUriExpected: googleCalendarRedirectUri(),
      appUrl: (process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.glossbossatx.com').replace(/\/$/, ''),
    },
