...
export default async function OwnerSetupCenterPage() {
  ...
  const [stripe, mediaRes, serviceRes, reviewRes, lifecycleProbe, exceptionProbe, syncRunsProbe, closeoutProbe, estimatesProbe] = await Promise.all([
  ...
  const financialMigrationsReady = !exceptionProbe.error && !lifecycleProbe.error && !syncRunsProbe.error && !closeoutProbe.error && !estimatesProbe.error;
  ...
  const checks: Readiness[] = [
    ...
    },
    {
      ...
      important: true,
      detail: financialMigrationsReady ? 'Audit, lifecycle, and exception tables are available.' : 'Apply Supabase migrations 000079 through 000085 before production deployment.',
