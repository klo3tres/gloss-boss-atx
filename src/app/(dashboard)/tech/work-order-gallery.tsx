...
export function WorkOrderGallery({
  ...
  const movePhoto = async (p: WorkOrderGalleryPhoto, newCategory: string, newPhase?: 'before' | 'after') => {
    setMoving(true);
    setMsg(null);
    try {
      const res = await fetch('/api/tech/job-media-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: p.id,
      ...
      const j = (await res.json().catch(() => ({}))) as { error?: string };
      if (!res.ok) {
        setMsg(j.error ?? 'Move failed.');
        return;
      }
      
      // Update active photo category/phase details in local state immediately
      setActive((prev) => {
      ...
      setMsg('Photo moved successfully.');
      router.refresh();
    } catch {
      setMsg('Failed to move photo.');
    } finally {
      setMoving(false);
    }
  };
  ...
  return (
    <section className="gb-glass gb-premium-card rounded-2xl border border-gold/25 p-4 shadow-[0_0_28px_rgba(212,175,55,0.08)] sm:p-5">
      ...
      {msg ? <p className="mt-2 text-xs text-zinc-400 bg-black/40 px-3 py-1.5 rounded-lg border border-gold/15 inline-block">{msg}</p> : null}
      ...
      {active ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-md" onClick={() => setActive(null)}>
          ...
          >
            ...
            {/* Category / Phase shifting controls */}
            <div className="mb-4 flex flex-wrap items-center gap-4 border-t border-b border-white/5 py-3.5">
              <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400">
                Shift photo category slot:
              </span>
              
              {/* Phase Select */}
              <div className="flex items-center gap-2">
                <span className="text-[9px] text-zinc-500 font-bold uppercase">Phase:</span>
                <select
                  value={active.phase || 'before'}
                  disabled={moving}
                  onChange={(e) => void movePhoto(active, active.category, e.target.value as 'before' | 'after')}
                  className="bg-black border border-white/15 text-white rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-gold/40"
                >
                  <option value="before">Before Restoration</option>
                  <option value="after">After Restoration</option>
                </select>
              </div>

              {/* Slot Select */}
              <div className="flex items-center gap-2">
                <span className="text-[9px] text-zinc-500 font-bold uppercase">Slot Category:</span>
                <select
                  value={active.category}
                  disabled={moving}
                  onChange={(e) => void movePhoto(active, e.target.value, active.phase)}
                  className="bg-black border border-white/15 text-white rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-gold/40"
                >
