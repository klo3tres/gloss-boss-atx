...
export function AppointmentNotificationPolicyPanel({ policy, canEdit }: { policy: AppointmentNotificationPolicy; canEdit: boolean }) {
  ...
  return <form action={updateAppointmentNotificationPolicyAction} className="rounded-2xl border border-white/10 bg-black/45 p-5">
    ...
    <p className="mt-2 text-xs text-zinc-400">Controls acknowledgment, on-the-way, late-start, and estimated-duration escalations. The monitor checks every five minutes while an admin or technician dashboard is active, with a daily fallback sweep. Closed, flexible, rescheduled, and approved-delay jobs are excluded.</p>
