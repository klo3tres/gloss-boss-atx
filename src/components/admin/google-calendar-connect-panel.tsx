...
export function GoogleCalendarConnectPanel() {
  ...
  return (
    <div className='rounded-2xl border border-white/10 bg-black/45 p-5'>
      <p className='text-[10px] font-black uppercase tracking-[0.2em] text-gold-soft'>Calendar Operator</p>
      ...
      {!status ? (
        ...
        <div className='mt-3 space-y-1 text-xs'>
          <p className='text-emerald-300'>Connected as {status.email ?? 'Google account'}.</p>
          <p className='text-zinc-500'>Push: bookings auto-sync · Pull: {status.lastPullAt ? new Date(status.lastPullAt).toLocaleString() : 'never'}</p>
          {status.lastError ? <p className='text-rose-300'>Last error: {status.lastError}</p> : null}
        </div>
      ) : (
        <p className='mt-3 text-xs text-zinc-400'>Not connected — connect to sync Titan with Google Calendar.</p>
      )}
      <div className='mt-4 flex flex-wrap gap-2'>
        <a href='/api/admin/google-calendar/connect' className='rounded-xl bg-gold px-4 py-2 text-[10px] font-black uppercase text-black'>
          ...
          <>
            <button
              ...
              onClick={() => {
                void (async () => {
                  setBusy(true);
                  setMsg(null);
                  const res = await fetchWithTimeout('/api/admin/google-calendar/pull', { method: 'POST', credentials: 'same-origin', timeoutMs: 60000 });
            ...
            <button
              ...
              onClick={() => {
                void (async () => {
                  setBusy(true);
                  const res = await fetchWithTimeout('/api/admin/google-calendar/status', { method: 'DELETE', credentials: 'same-origin', timeoutMs: 15000 });
                  setBusy(false);
                  if (!res.ok) {
