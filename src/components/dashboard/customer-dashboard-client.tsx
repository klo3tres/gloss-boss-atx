    ...
    <div className="space-y-8 rounded-3xl p-1 sm:p-2">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          {/* Loyalty Stepper Panel */}
          <GlassCard className="border-gold/20">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <SectionEyebrow>Loyalty Status</SectionEyebrow>
              <Award className="h-4.5 w-4.5 text-gold" />
            </div>
            
            <div className="mt-4 text-center">
              <p className="text-4xl font-mono font-black text-white">{loyaltyVisits}</p>
              <p className="text-[10px] font-black uppercase tracking-wider text-zinc-400 mt-1">Completed Detail Services</p>
            </div>

            {/* Stepper Progress Visualizer */}
            <div className="mt-6 space-y-3">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-500 font-semibold">Tier Progress</span>
                <span className="font-mono text-gold-soft font-bold">{currentStep}/{loyaltyTarget} visits</span>
              </div>
              
              <div className="h-2 w-full rounded-full bg-white/5 overflow-hidden">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-gold/50 to-gold transition-all duration-500"
                  style={{ width: `${loyaltyPercent}%` }}
                />
              </div>

              {/* Progress Node markers */}
              <div className="flex justify-between items-center text-[10px] text-zinc-500 pt-1">
                {[1, 2, 3, 4, 'Reward!'].map((val, idx) => {
                  const nodeStep = idx + 1;
                  const isDone = currentStep >= nodeStep || (idx === 4 && loyaltyVisits >= loyaltyTarget && currentStep === 0);
                  return (
                    <span
                      key={idx}
                      className={`font-bold transition ${isDone ? 'text-gold-soft' : 'text-zinc-600'}`}
                    >
                      {idx === 4 ? '★' : `•`}
                    </span>
                  );
            ...
            <p className="mt-6 text-center text-xs text-zinc-500 italic leading-relaxed">
            ...
            <Link
              href="/book"
              className="mt-5 inline-flex w-full items-center justify-center rounded-xl bg-gradient-to-r from-gold via-gold-soft to-gold py-3.5 text-xs font-black uppercase tracking-widest text-black shadow-[0_0_24px_rgba(212,175,55,0.2)] hover:brightness-110 transition duration-300"
            >
