...
import { Copy, Gift, Link2, Mail, MessageSquare } from 'lucide-react';
...
export function CustomerReferralCard({
  ...
  const remaining = Math.max(0, threshold - completedReferrals);
  ...
  return (
    <section className="rounded-3xl border border-gold/20 bg-black/55 p-5 backdrop-blur-xl">
      ...
      <p className="mt-2 text-sm text-zinc-400">
        {rewardRules ?? 'Share your link. When friends book and complete, you unlock Gloss Boss rewards.'}
      </p>
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={shareSms} className="inline-flex items-center gap-1.5 rounded-xl border border-white/15 px-3 py-2 text-[10px] font-bold uppercase text-zinc-300">
          <MessageSquare className="h-3.5 w-3.5" /> Share SMS
        </button>
        <button type="button" onClick={shareEmail} className="inline-flex items-center gap-1.5 rounded-xl border border-white/15 px-3 py-2 text-[10px] font-bold uppercase text-zinc-300">
          <Mail className="h-3.5 w-3.5" /> Share email
        </button>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-[10px] uppercase text-zinc-500 sm:grid-cols-4">
        <div className="rounded-xl border border-white/5 p-2"><p>Sent</p><p className="text-lg font-black text-white">{sentReferrals ?? 0}</p></div>
        ...
        <div className="rounded-xl border border-white/5 p-2"><p>Rewards</p><p className="text-lg font-black text-gold-soft">{rewardsAvailable ?? 0}/{rewardsEarned ?? 0}</p></div>
      </div>
      <div className="mt-4">
        <div className="flex justify-between text-[10px] uppercase text-zinc-500">
          <span>{remaining > 0 ? `${remaining} away from next reward` : 'Next reward unlocked!'}</span>
