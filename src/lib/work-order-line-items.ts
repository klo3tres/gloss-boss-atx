...
export function readCustomLineItems(job: Row): WorkOrderLineItem[] {
  ...
  for (const item of raw) {
    ...
    const amountCents = typeof o.amountCents === 'number' ? o.amountCents : typeof o.amount_cents === 'number' ? o.amount_cents : Number(o.amountCents ?? o.amount_cents);
...
export function customLineItemsAsReceiptRows(
  ...
  return readCustomLineItems(job).map((item) => ({
    name: item.label,
